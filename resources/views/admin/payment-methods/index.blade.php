        ...
        <table class="min-w-full divide-y divide-white/5">
            <thead class="bg-white/5">
                <tr>
                    ...
                    <th class="px-8 py-5 text-left text-xs font-black text-slate-400 uppercase tracking-widest">Specific Packages</th>
                ...
                <tr class="hover:bg-white/5 transition-all duration-200 group">
                    <td class="px-8 py-6 whitespace-nowrap">
                        <div class="flex items-center">
                            @if($method->icon)
                            <img src="{{ $method->icon_url }}" alt="{{ $method->name }}" class="h-12 w-12 object-contain rounded-2xl border border-white/10 bg-white/5 p-1.5 flex-shrink-0 group-hover:scale-105 transition-transform">
                            @else
                    ...
                    <td class="px-8 py-6 whitespace-nowrap">
                        @php
                            $types = $method->applicable_to ? explode(',', $method->applicable_to) : ['all'];
                        @endphp
                        <div class="flex flex-wrap gap-1.5">
                            @if(in_array('all', $types))
                                <span class="px-2 py-0.5 text-[10px] font-black uppercase tracking-wider bg-blue-500/10 border border-blue-500/30 text-blue-400 rounded-lg">All</span>
                            @else
                                @foreach($types as $t)
                                <span class="px-2 py-0.5 text-[10px] font-black uppercase tracking-wider {{ $t === 'hosting' ? 'bg-cyan-500/10 border border-cyan-500/30 text-cyan-400' : ($t === 'domain' ? 'bg-emerald-500/10 border border-emerald-500/30 text-emerald-400' : 'bg-purple-500/10 border border-purple-500/30 text-purple-400') }} rounded-lg">{{ $t }}</span>
                                @endforeach
                            @endif
                        </div>
                    </td>
                    <td class="px-8 py-6 max-w-xs">
                        @if($method->applicable_providers)
                            <p class="text-xs text-slate-400 font-medium max-w-[200px]" title="{{ $method->applicable_providers }}">Providers: {{ $method->applicable_providers }}</p>
                        @else
                            <span class="text-xs text-slate-600">All services</span>
                        @endif
                    </td>
                    <td class="px-8 py-6 max-w-xs">
                        @if($method->applicable_package_ids)
                            <p class="text-xs text-slate-400 font-medium max-w-[200px]" title="{{ $method->applicable_package_ids }}">Package IDs: {{ $method->applicable_package_ids }}</p>
                        @else
                            <span class="text-xs text-slate-600">All packages</span>
                        @endif
                    </td>
                    <td class="px-8 py-6 max-w-xs">
