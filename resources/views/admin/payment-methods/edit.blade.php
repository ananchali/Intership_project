...
<div class="max-w-3xl mx-auto">
    <div class="mb-8">
        ...
        <h1 class="text-3xl font-bold text-gray-900 mt-4">Edit Bank Method: {{ $paymentMethod->name }}</h1>
        <p class="text-gray-500 mt-1">Update the account details for this transfer method</p>
    </div>
        ...
        <form action="{{ route('admin.payment-methods.update', $paymentMethod->id) }}" method="POST">
            ...
            <div class="grid grid-cols-1 gap-6">
                <div>
                    ...
                    <input type="text" name="name" value="{{ $paymentMethod->name }}" required 
                ...
                <div>
                    ...
                    <input type="text" name="account_number" value="{{ $paymentMethod->account_number }}" required 
                ...
                <div>
                    ...
                    <input type="text" name="account_name" value="{{ $paymentMethod->account_name }}" required 
                           class="mt-1 block w-full rounded-lg border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 border p-3 bg-gray-50">
                </div>

                <div>
                    <label class="block text-sm font-bold text-gray-700 mb-1">Instructions (optional)</label>
                    <textarea name="instructions" rows="4" class="mt-1 block w-full rounded-lg border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 border p-3 bg-gray-50">{{ $paymentMethod->instructions }}</textarea>
                </div>

                <div class="flex items-center bg-gray-50 p-4 rounded-lg border border-gray-100">
                    <input type="checkbox" name="is_active" id="is_active" {{ $paymentMethod->is_active ? 'checked' : '' }} 
                           class="h-5 w-5 text-blue-600 focus:ring-blue-500 border-gray-300 rounded cursor-pointer">
                    <label for="is_active" class="ml-3 block text-sm font-medium text-gray-700 cursor-pointer">Method is Active</label>
                </div>
