...
<div class="max-w-7xl mx-auto px-4 py-12">
    ...
    @php $isServicePkg = $selectedPackage->type === 'services'; @endphp
    <div class="relative bg-gradient-to-r from-blue-900 to-purple-900 rounded-[2.5rem] shadow-xl shadow-blue-900/10 p-8 md:p-10 mb-10 text-white overflow-hidden border border-blue-950">
        ...
        <div class="relative z-10 flex flex-col md:flex-row items-center justify-between gap-6">
            <div>
                ...
                <h2 class="text-3xl font-black mb-2 tracking-tight">Continue Your {{ $isServicePkg ? 'Service' : 'Hosting' }} Order</h2>
                <p class="text-blue-100 max-w-xl font-medium">
                    @if($isServicePkg)
                        You selected <strong class="text-white">{{ $selectedPackage->name }}</strong>. Choose a service level to complete your order.
                    @else
                        You selected the <strong class="text-white">{{ $selectedPackage->name }}</strong> hosting package. Finish setting up your domain to secure this offer.
                    @endif
                </p>
            </div>
            <a href="{{ route('orders.yegara-flow', ['step' => 2, 'package_id' => $selectedPackage->id]) }}" 
               class="mt-4 md:mt-0 bg-white hover:bg-blue-50 text-blue-900 font-extrabold px-8 py-4 rounded-2xl transition-all shadow-lg hover:shadow-white/20 flex items-center gap-2 group whitespace-nowrap">
                @if($isServicePkg) Choose Level @else Choose Domain @endif
    ...
    <!-- Quick Actions: Start New Order -->
    <div class="mb-10">
        <h2 class="text-2xl font-black text-gray-900 tracking-tight mb-6">Start a New Order</h2>
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
            <a href="{{ route('packages.index', ['type' => 'hosting']) }}" class="bg-gradient-to-br from-blue-500 to-blue-700 rounded-2xl p-8 text-white hover:shadow-xl hover:shadow-blue-500/20 transition-all group">
                <div class="w-14 h-14 bg-white/15 rounded-xl flex items-center justify-center mb-4">
                    <svg class="w-7 h-7" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 8h14M5 8a2 2 0 110-4h14a2 2 0 110 4M5 8v10a2 2 0 002 2h10a2 2 0 002-2V8m-9 4h4"/></svg>
                </div>
                <h3 class="text-xl font-bold mb-2">Hosting</h3>
                <p class="text-blue-100 text-sm font-medium group-hover:text-white transition-colors">Browse hosting plans &rarr;</p>
            </a>
            <a href="{{ route('packages.index', ['type' => 'domain']) }}" class="bg-gradient-to-br from-emerald-500 to-emerald-700 rounded-2xl p-8 text-white hover:shadow-xl hover:shadow-emerald-500/20 transition-all group">
                <div class="w-14 h-14 bg-white/15 rounded-xl flex items-center justify-center mb-4">
                    <svg class="w-7 h-7" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 12a9 9 0 01-9 9m9-9a9 9 0 019 0 9 9 0 01-9 9z"/></svg>
                </div>
                <h3 class="text-xl font-bold mb-2">Domains</h3>
                <p class="text-emerald-100 text-sm font-medium group-hover:text-white transition-colors">Register or transfer a domain &rarr;</p>
            </a>
            <a href="{{ route('packages.index', ['type' => 'services']) }}" class="bg-gradient-to-br from-purple-500 to-purple-700 rounded-2xl p-8 text-white hover:shadow-xl hover:shadow-purple-500/20 transition-all group">
                <div class="w-14 h-14 bg-white/15 rounded-xl flex items-center justify-center mb-4">
                    <svg class="w-7 h-7" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4"/></svg>
                </div>
                <h3 class="text-xl font-bold mb-2">Services</h3>
                <p class="text-purple-100 text-sm font-medium group-hover:text-white transition-colors">Explore organizational services &rarr;</p>
            </a>
        </div>
    </div>
