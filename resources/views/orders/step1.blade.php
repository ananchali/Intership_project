...
<h1 class="page-title">Choose Your Package</h1>
<p class="page-subtitle">Select the perfect plan for your website or organization needs.</p>
...
@php
    ...
    $typeLabels = [
        ...
        'services' => 'Organizational Services',
    ];
@endphp

@foreach($grouped as $type => $typePackages)
<h2 class="page-subtitle" style="font-size: 1.5rem; font-weight: 700; color: var(--text-main); margin-top: 2rem; margin-bottom: 1rem;">
    {{ $typeLabels[$type] ?? ucfirst($type) . ' Packages' }}
</h2>
<div class="packages-grid">
    ...
    <div class="package-card">
        ...
        <div class="pkg-price">{{ number_format($package->price) }} {{ $package->currency }} @if($type === 'hosting')<span>/mo</span>@endif</div>
            ...
            @if(is_array($package->features))
                @foreach($package->features as $feature)
                    <li>
                        ...
                        {{ is_string($feature) ? $feature : (is_array($feature) ? json_encode($feature) : '') }}
                    </li>
                @endforeach
            @endif
        </ul>
